---
description: Package a DaVinci Resolve Fusion template — zips the Edit/ folder and renames it Edit.drfx
---
...
3. Delete `$ARGUMENTS\Edit.drfx` if it already exists (clean rebuild).
4. Crée le ZIP manuellement avec `ZipArchive` pour garantir les forward slashes et les entrées de dossiers explicites (requis par DaVinci Resolve) :
   ...
   $drfxPath     = Join-Path (Resolve-Path $templateName).Path "Edit.drfx"
   ...
   Rename-Item -Path $zipTemp -NewName "Edit.drfx"
